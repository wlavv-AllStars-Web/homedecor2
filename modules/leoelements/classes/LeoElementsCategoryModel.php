...
class LeoElementsCategoryModel extends ObjectModel
{
    ...
    public function toggleStatusMT($field)
    {
        ...
        $result = Db::getInstance()->getRow('SELECT '.$field.' from  `'._DB_PREFIX_.'leoelements_category_shop` '.$where);
        $value = $result[$field]==1?0:1;

        if ($value == 1) {
            Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'leoelements_category_shop` set '.$field.' = "0" WHERE id_shop="'.$id_shop.'"');
            Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'leoelements_category` set '.$field.' = "0"');
        }
        
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'leoelements_category_shop` set '.$field.' = "'.$value.'" '.$where);
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'leoelements_category` set '.$field.' = "'.$value.'" '.$where1);
