...
class AdminLeoElementsImagesController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->className      = 'LeoElementsImages';
    ...
    public function renderList()
    {
        ...
        $image_uploader->setMultiple(true)->setUseAjax(true)->setUrl(Context::getContext()->link->getAdminLink('AdminLeoElementsImages') . '&ajax=1&action=addimage&leo_controller=live_theme_edit'); // url upload image
        ...
        $tpl->assign(array(
            ...
            'imgManUrl' => Context::getContext()->link->getAdminLink('AdminLeoElementsImages'),
            'token' => $this->token,
    ...
    public function ajaxProcessManageImage()
    {
        ...
        $image_uploader->setMultiple(true)->setUseAjax(true)->setUrl(Context::getContext()->link->getAdminLink('AdminLeoElementsImages') . '&ajax=1&action=addimage&imgDir=' . $this->folder_name);
    ...
    public function ajaxProcessDeleteImage()
    {
        if (($img_name = Tools::getValue('imgName', false)) !== false) {
            $link = $this->img_path;
            $this->icon_path = $link = str_replace('modules/leoelements/images', 'modules/leoelements/icon', $link);
    ...
    public function getIconList($sortBy)
    {
        ...
        foreach ($images as &$file) {
            ...
            $result[] = array(
                'name' => $fileInfo['basename'],
                'link' => str_replace(
                    'modules/leoelements/images',
                    'modules/leoelements/icon',
                    $this->img_url
                ) . $fileInfo['basename']
            );
        }
